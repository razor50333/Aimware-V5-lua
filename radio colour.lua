...
local chat_spam = gui.Combobox( tab, "Chicken.RaidoColor.spam", "Enable mode", "off", "Spam");
...
callbacks.Register("Draw", function()
    ...
    if globals.CurTime() > delay and chat_spam:GetValue() and string.len(message) > 0 then
        local color = RColor_index[color_picker:GetValue() + 1]
        SendColoredRadioMessage(RColor[color], custom_msg:GetValue())
