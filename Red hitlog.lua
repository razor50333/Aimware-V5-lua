local activeHitLogs = {};
local font = draw.CreateFont('verdana bold', 15, 10);
...
local function showLog(count, color, text, layer)
    ...
    if globals.RealTime() < layer.delay then
        if layer.x_pad < mw then layer.x_pad = layer.x_pad + (mw - layer.x_pad) * 0.04 end
        if layer.x_pad > mw then layer.x_pad = mw end
        if layer.x_pad > mw / 2.09 then
...
callbacks.Register('FireGameEvent', function(e)
    ...
    if isHurt then
        ...
        if (localPlayer:GetIndex() == attacker:GetIndex()) then
            add(5,
                { 255, 255, 255, "Hit " },
                { 255, 0, 15, string.sub(user:GetName(), 0, 28) },
                { 255, 255, 255, " in the " },
                { 255, 0, 15, HitGroup(e:GetInt('hitgroup')) },
                { 255, 255, 255, " for " },
                { 255, 0, 15, damageDone },
                { 255, 255, 255, " damage (" },
                { 255, 0, 15, remainingHealth .. " health remaining" },
                { 255, 255, 255, ")" })
        end
